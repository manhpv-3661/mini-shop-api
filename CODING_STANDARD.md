...
| `C030` (rethrow biến lỗi gốc)                     | _"Use custom error classes instead of throwing variables"_ ở dòng `throw error;` trong `follows.service.ts`/`favorites.service.ts` (và từ PR09: `products.service.ts` `replaceProductImage`, `categories.service.ts` `deleteCategory`)                                                                                                                                                           | Đây **không phải** throw một biến string/object tuỳ tiện — `error` ở đây là chính exception đã bắt được từ `catch (error)`, được rethrow nguyên trạng khi nó không phải trường hợp cụ thể đang xử lý riêng (unique/FK violation, hoặc ở PR09 `replaceProductImage`: sau khi đã chạy xong hành động bù trừ cần thiết — xóa file mới vừa ghi khi transaction lỗi, xem mục 6). Heuristic chỉ thấy cú pháp `throw <identifier>` nên hiểu nhầm là throw giá trị thô, không phân biệt được `identifier` đó đã là một `Error` instance hợp lệ từ catch clause hay chưa.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `C014` (PR06, áp dụng chung mọi service có sẵn)   | _"Direct instantiation of 'Logger'"_ ở `auth.service.ts` (và đã có sẵn trước đó ở `mail.processor.ts`, `notification-dispatcher.service.ts`, `all-exceptions.filter.ts`; `users.service.ts` thêm ở PR07; `categories.service.ts`/`products.service.ts` thêm ở PR09; `cart.service.ts` thêm ở PR11; `orders.service.ts` thêm ở PR12; `product-suggestions.service.ts` thêm ở PR14)                | `new Logger(ClassName.name)` là pattern chính thức NestJS khuyến nghị cho field logger riêng của từng class — không phải service dependency cần qua DI như `Repository`/`ConfigService`. Bắt buộc DI ở đây sẽ phá vỡ context name tự động (`ClassName.name`) mà `Logger` dùng để gắn nhãn log.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
