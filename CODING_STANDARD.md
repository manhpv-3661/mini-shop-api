...
chat ──depends on──▶ users, auth (chỉ JwtModule/JwtStrategy export riêng, verify JWT thô ở WebSocket handshake — mục 24)
...
Đo bằng `npm run test:e2e:cov` (config `collectCoverageFrom` trong `test/jest-e2e.json` giới hạn vào `src/modules/comments/**/*.ts`). Kết quả đo thực tế: `comments.service.ts` và `comments.controller.ts` đều 100% statement / 100% function. Branch coverage báo 75-82% ở cả 2 file, nhưng khi soi từng nhánh cụ thể (`coverage-e2e/coverage-final.json`, field `branchMap`+`b`):
...
- **`CommentsService`: chỉ 1 nhánh nghiệp vụ thật chưa phủ** — ternary tính `authorFollowing` trong `toResponseDto` (`currentUserId && currentUserId !== comment.authorId ? isFollowing(...) : false`). Nhánh `true` (gọi `isFollowing`) không có đường nào chạm tới qua e2e vì `toResponseDto` chỉ được gọi từ `create()`, nơi `currentUserId` luôn chính là tác giả comment vừa tạo — về cấu trúc không thể khác `comment.authorId` qua bất kỳ route hiện có nào. Đã test ở `comments.service.spec.ts` (unit, gọi thẳng `toResponseDto(comment, 'viewer-id')` với `isFollowing` mock trả `true`). Nhánh `if (!comment)` (`NotFoundException`) trong `deleteByIdForArticle` — tưởng chưa phủ ở lần đo trước — thực ra **đã** được e2e phủ (`returns 404 when deleting an unknown comment id`). Mọi ternary/`if` nghiệp vụ khác (`toListResponseDto`, nhánh `ForbiddenException` của `deleteByIdForArticle`) đều phủ cả 2 chiều qua e2e.
...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `C033`                                            | _"Service vừa dùng Repository vừa dùng DataSource — pattern không nhất quán"_, và _"method gọi trực tiếp `manager.getRepository()`"_                                                                                                                                                                                                                                                                                                                                                                                         | `UsersService`/`AttachmentsService` (và từ PR09: `ProductsService`/`CategoriesService`; từ PR11: `CartService`; từ PR12-13: `OrdersService` — toàn bộ method đều bọc `dataSource.transaction()`, kể cả 2 method GET list không transaction (`listForCustomer`/`listForAdmin`) vẫn dùng `dataSource.getRepository()` trực tiếp thay vì `@InjectRepository`, có chủ đích để cùng 1 cặp helper `fetchOrderItems`/`fetchOrderHistory`/`loadOrderResponse` dùng được cả trong và ngoài transaction qua tham số `manager?: EntityManager`; từ PR14: `ProductSuggestionsService` — `create`/`listForCustomer`/`listForAdmin`/`getDetailForAdmin` dùng `@InjectRepository` cho query đơn không cần transaction, `reviewByAdmin` dùng `manager.getRepository(ProductSuggestion)` bên trong `dataSource.transaction()` vì cần khoá row rồi ghi cùng transaction đó; từ PR15: `ChatService`/`ChatAccessService` — không có method nào inject `Repository` (mọi thao tác đều qua `dataSource.getRepository()`/`manager?.getRepository()` để cùng 1 helper như `loadConversationResponse`/`assertCanReadConversation` dùng được cả trong và ngoài transaction, giống hệt lý do của `OrdersService` ở trên) dùng `Repository` inject cho query đơn, và `manager.getRepository(Entity)` bên trong `dataSource.transaction()` khi cần ghi nhiều bảng atomic trong 1 transaction (xem mục 6). `manager.getRepository()` là cách chuẩn của TypeORM để lấy Repository _đúng transaction đang chạy_ — không dùng nó thì transaction rỗng (bug thật đã xảy ra, xem mục 6). Không phải lỗi kiến trúc. |
...
| `C018`                                            | _"Error logging should use structured format / should explain what happened / should provide guidance"_ ở các block `catch` trong `users.service.ts`, `follows.service.ts`, `favorites.service.ts`, `articles.service.ts` (và từ PR09: `categories.service.ts`, `products.service.ts` ở các hàm `toConflictOrRethrow`/`deleteCategory`)                                                                                                                                                                                      | Các block này **rethrow chứ không log**: `catch (error) { throw this.toConflictOrRethrow(error) }` (hoặc `if (isUniqueViolation(error)) { throw new ConflictException(...) } throw error;`) chuyển lỗi unique-violation thành `ConflictException`, còn lỗi khác thì rethrow nguyên trạng (mục 7, mục 16 áp dụng tương tự cho `follows`/`favorites`/`articles`). Log ở đây là sai chỗ theo 2 lý do: (1) `ConflictException` là kết quả nghiệp vụ mong đợi (user follow/favorite trùng), không phải sự cố cần log; (2) lỗi thật sự bất thường đã được `AllExceptionsFilter` log tập trung một lần (mục 9) — log thêm ở service tạo log trùng, cùng 1 lỗi xuất hiện 2 dòng. Sunlint chỉ thấy từ khoá `catch` + `error` nên tưởng là điểm logging.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `C030` (rethrow biến lỗi gốc)                     | _"Use custom error classes instead of throwing variables"_ ở dòng `throw error;` trong `follows.service.ts`/`favorites.service.ts` (và từ PR09: `products.service.ts` `replaceProductImage`, `categories.service.ts` `deleteCategory`; từ PR15: `chat.gateway.ts`'s `handleConnection()` — rethrow lại đúng lỗi verify JWT vừa bắt được, sau khi đã `client.disconnect(true)`, để nhánh `.catch()` thứ 2 trên cùng Promise vẫn nhận được lỗi thật thay vì bị nuốt mất)                                                       | Đây **không phải** throw một biến string/object tuỳ tiện — `error` ở đây là chính exception đã bắt được từ `catch (error)`, được rethrow nguyên trạng khi nó không phải trường hợp cụ thể đang xử lý riêng (unique/FK violation, hoặc ở PR09 `replaceProductImage`: sau khi đã chạy xong hành động bù trừ cần thiết — xóa file mới vừa ghi khi transaction lỗi, xem mục 6). Heuristic chỉ thấy cú pháp `throw <identifier>` nên hiểu nhầm là throw giá trị thô, không phân biệt được `identifier` đó đã là một `Error` instance hợp lệ từ catch clause hay chưa.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `C035`                                            | _"Error logging should use structured format (object) instead of string concatenation"_ ở `mail.processor.ts`, `notification-dispatcher.service.ts`, `seed.ts` (đã có từ trước PR09; PR09 thêm 1 vị trí ở `attachments.service.ts` `deleteFileByStorageKey`, cùng style)                                                                                                                                                                                                                                                     | Cùng nhóm với `S037` ở trên — gợi ý đổi toàn bộ `Logger` sang structured/object logging là hardening áp dụng cho **mọi** service hiện có, không phải lỗi riêng của 1 dòng. `attachments.service.ts` chỉ viết đúng theo convention string-interpolation đã dùng nhất quán ở mọi service khác trong repo (`UsersService`, `AuthService`...) — đổi riêng 1 chỗ sẽ tạo 2 style logging khác nhau trong cùng codebase. Cân nhắc làm đồng loạt ở 1 pull riêng về logging/observability, không sửa lẻ tẻ từng PR.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `C041`                                            | _"Variable 'NOTIFICATION_SECRET_KEY_PROVIDER' appears to contain hardcoded sensitive information"_ trong `notification-secret/notification-secret.constants.ts` (chuyển từ `notifications.constants.ts` ở PR06)                                                                                                                                                                                                                                                                                                              | Đây là DI token (chuỗi định danh provider, giống `REDIS_CLIENT` đã có sẵn), không phải secret thật — giá trị chuỗi trùng tên biến chỉ để làm token duy nhất cho `@Inject()`. Heuristic thấy cụm `SECRET_KEY` trong tên biến nên tưởng là credential bị hardcode.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `S004`                                            | _"Sensitive data logging... may expose sensitive field 'email'"_ ở `mail.processor.ts` dòng `Sent ${notification.eventType} email for notification ${notificationId}`                                                                                                                                                                                                                                                                                                                                                        | Log chỉ nội suy `eventType` và `notificationId` — không có địa chỉ email nào trong chuỗi. Heuristic thấy từ "email" xuất hiện trong văn bản log (mô tả hành động "sent ... email") nên tưởng nhầm là đang log giá trị email thật.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `C024`                                            | _"Magic string 'NODE_ENV'/'utf8' should be extracted as named constant"_ ở `notification-dispatcher.service.ts` và `notification-secret-cipher.util.ts` (và từ PR09: `'NODE_ENV'` ở `attachments-cleanup.service.ts`, `'ENOENT'` ở `attachments.service.ts`)                                                                                                                                                                                                                                                                 | `'NODE_ENV'` là tên biến môi trường chuẩn của Node.js, `'utf8'` là tham số encoding chuẩn của `Buffer`/`string` API, `'ENOENT'` là mã lỗi hệ điều hành chuẩn của `fs` API (`NodeJS.ErrnoException.code`) — tất cả là hằng số ngôn ngữ/nền tảng, không phải magic value nghiệp vụ cần gom vào `constants/` (khác với vd `MAX_EMAIL_NOTIFICATION_ATTEMPTS`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `C024` (PR09, khác nhóm platform-constant ở trên) | _"Magic string 'storageKey'/'mimeType'/'sizeBytes' should be extracted as named constant"_ ở `attachments.service.ts` (option `select` của TypeORM); _"Magic string 'DESC' should be extracted as named constant"_ ở `categories.service.ts` (`.addOrderBy(...)`); _"Magic string 'image/jpeg'/'image/png'/'image/webp' should be extracted as named constant"_ ở `attachment-signature.util.ts`; _"Magic string 'customerId' should be extracted as named constant"_ ở `chat-authorization.util.ts` (PR15)                  | Tham số **bắt buộc là literal string** theo API/type đã định nghĩa sẵn: `select: { storageKey: true }` là tên property của entity (TypeScript tự kiểm đúng tên, không phải giá trị nghiệp vụ tuỳ ý), `'DESC'`/`'ASC'` là 2 giá trị cố định `addOrderBy()` của TypeORM chấp nhận (kiểu `'ASC' \| 'DESC'`), và các literal MIME trong `attachment-signature.util.ts` chính là giá trị của `AttachmentMimeType` (suy ra từ `ALLOWED_ATTACHMENT_MIME_TYPES` ở `constants/`) — hàm cần trả về đúng 1 trong 3 literal đó nên gõ trực tiếp rõ ràng hơn tra ngược qua index mảng. Gom vào `constants/` chỉ thêm 1 lớp gián tiếp không cần thiết cho các API/type đã có type-safety riêng. Cùng nhóm: `conversation.customerId` trong `canAccessConversation()` (`chat-authorization.util.ts`) là truy cập property của `Pick<ChatConversation, 'customerId'>` — tên field đã được TypeScript kiểm đúng, không phải chuỗi nghiệp vụ tuỳ ý cần gom hằng số.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `C041`                                            | _"Variable 'DUMMY_PASSWORD_HASH' appears to contain hardcoded sensitive information"_ ở `auth/constants/auth.constants.ts`                                                                                                                                                                                                                                                                                                                                                                                                   | Không phải password/hash của tài khoản thật — đây là bcrypt hash cố định của một chuỗi không ai dùng làm mật khẩu thật, chỉ để `bcrypt.compare()` trong `AuthService.login()` luôn chạy đủ thời gian kể cả khi email không tồn tại (chống timing attack dò email đã đăng ký). Heuristic thấy hậu tố `_HASH`/tiền tố `PASSWORD` trong tên biến nên tưởng là secret bị lộ.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `S042` (PR06)                                     | _"JWT token created without expiration configuration. Add expiresIn option"_ ở `auth.service.ts`, dòng `this.jwtService.sign(payload)`                                                                                                                                                                                                                                                                                                                                                                                       | `expiresIn` đã khai một lần duy nhất trong `AuthModule`'s `JwtModule.registerAsync({ signOptions: { expiresIn: config.getOrThrow('JWT_EXPIRES_IN') } })` — mọi `JwtService.sign()` tiêm từ module này tự động kế thừa TTL đó. Heuristic chỉ đọc được call site `sign(payload)` đơn lẻ, không thấy cấu hình global của module nên tưởng token không có hạn.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `S011` (PR06)                                     | _"Variable 'authToken' uses weak random generation for security purposes"_ ở `auth.service.ts`, dòng `authTokenRepository.create({...})` trong `createAuthToken()`                                                                                                                                                                                                                                                                                                                                                           | Random generation thật sự nằm ngay phía trên, `randomBytes(AUTH_TOKEN_BYTES)` từ Node `crypto` (đổi tên từ `EMAIL_VERIFICATION_TOKEN_BYTES` ở PR07 vì hằng số này giờ dùng chung cho cả `PASSWORD_RESET`) — đã là CSPRNG đúng chuẩn, không phải `Math.random()`. Heuristic thấy tên biến `authToken` chứa chữ "token" nằm gần lệnh sinh ngẫu nhiên nên gán nhầm cảnh báo vào dòng khai báo entity thay vì dòng gọi `randomBytes()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `S004` (PR06, tái diễn PR07)                      | _"Sensitive data logging... may expose sensitive field 'email'/'authtoken'/'password'/'token'"_ ở nhiều dòng `this.logger.*()` trong `auth.service.ts` (`register`/`verifyEmail`/`login`/`logout`/`forgotPassword`/`resetPassword`) và `users.service.ts` (`changePassword`)                                                                                                                                                                                                                                                 | Cùng nguyên nhân với dòng `mail.processor.ts` ở trên: log chỉ nội suy `user.id`/`authToken.id`/`authToken.userId`/`userId` (UUID định danh row, không phải giá trị nhạy cảm) — heuristic khớp theo **từ** xuất hiện trong câu mô tả tiếng Anh ("wrong **password**", "verified **email**", "**token** revoked"), không phải giá trị thật được log. Không có raw token, password hay email nào lọt vào log.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `C014` (PR06, áp dụng chung mọi service có sẵn)   | _"Direct instantiation of 'Logger'"_ ở `auth.service.ts` (và đã có sẵn trước đó ở `mail.processor.ts`, `notification-dispatcher.service.ts`, `all-exceptions.filter.ts`; `users.service.ts` thêm ở PR07; `categories.service.ts`/`products.service.ts` thêm ở PR09; `cart.service.ts` thêm ở PR11; `orders.service.ts` thêm ở PR12; `product-suggestions.service.ts` thêm ở PR14; `chat.service.ts`/`chat.gateway.ts` thêm ở PR15 — gateway cũng dùng field logger riêng như mọi class khác, không phải trường hợp đặc biệt) | `new Logger(ClassName.name)` là pattern chính thức NestJS khuyến nghị cho field logger riêng của từng class — không phải service dependency cần qua DI như `Repository`/`ConfigService`. Bắt buộc DI ở đây sẽ phá vỡ context name tự động (`ClassName.name`) mà `Logger` dùng để gắn nhãn log.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `S044` (PR07)                                     | _"Sensitive operation 'resetPassword'/'changePassword' requires re-authentication before execution"_ ở `auth.controller.ts` (`resetPassword`) và `users.controller.ts` (`changePassword`)                                                                                                                                                                                                                                                                                                                                    | Heuristic chỉ khớp theo tên hàm ("reset"/"change" + "password"), không đọc được rằng mỗi endpoint **đã** có cơ chế tương đương re-auth: `resetPassword` đòi hỏi opaque token một lần lấy từ email (chứng minh quyền truy cập hộp thư, thay cho việc đã đăng nhập — đúng bản chất luồng quên mật khẩu, không có session nào để "re-authenticate"); `changePassword` đòi hỏi `currentPassword` đúng (verify bằng `bcrypt.compare` trong `UsersService.changePassword`) trước khi cho đổi. Cả hai đã thỏa mãn tinh thần "re-authentication", chỉ khác hình dạng với pattern heuristic tìm (một bước re-login riêng biệt).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `S025` (PR09, khác với dòng S025 chung ở trên)    | _"File upload missing server-side validation (type, size, content)"_ ở `admin-products.controller.ts`, dòng khai `replaceProductImage`                                                                                                                                                                                                                                                                                                                                                                                       | Khác các vị trí `S025` khác trong bảng này (thật sự chưa làm, ghi nhận từ PR#3) — validation ở đây **đã có đủ cả 3 lớp**: kích thước qua `limits.fileSize` của Multer trong `createProductImageUploadInterceptor()` (413 tự động), loại file qua `fileFilter` của interceptor (chặn nhanh theo MIME khai báo), và nội dung qua `AttachmentsService.validateImageFile()` (so chữ ký byte thật, chặn giả mạo — gọi từ `ProductsService.replaceProductImage()`, không nằm trong chính file controller này). Heuristic chỉ quét tĩnh file controller đang mở, không lần theo được lệnh gọi service ở file khác nên không thấy phần validate thật.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `@ApiResponse({ status: 400 })`, `@ApiResponse({ status: 409 })` (số ma thuật, PR06 `auth.controller.ts`)                                                              | `@ApiResponse({ status: HttpStatus.BAD_REQUEST })`, `@ApiResponse({ status: HttpStatus.CONFLICT })`                                   | Cùng `HttpStatus` enum ở trên, áp dụng luôn cho field `status` của `@ApiResponse` (mục 19) — không chỉ `@HttpCode`; một route khai nhiều `@ApiResponse` càng dễ gõ nhầm số nếu không dùng enum.       |
...
**`*.controller.ts` loại khỏi `collectCoverageFrom` từ PR07.** Cùng lý do với `.module.ts`: controller không chứa business logic (mục 3), chỉ gọi lại đúng 1 hàm service — verify bằng e2e (mục 11), không phải unit test giả gọi controller trực tiếp. `auth.controller.ts` (PR06, 1 file) chưa đủ để kéo branch coverage xuống dưới sàn nên chưa cần loại trừ; PR07 thêm `users.controller.ts` và nhiều route hơn vào `auth.controller.ts` (forgot/reset password) làm branch coverage tổng rơi xuống 63.79% (dưới sàn 65%) dù không thiếu test logic nào — đúng lúc để loại trừ, không phải hạ threshold.
...
**`@MinLength(1)` không chặn được chuỗi toàn khoảng trắng.** Mentor review PR5 (binhpt-3177) chỉ ra: `"   "` (toàn dấu cách) có `length` là 3, vượt qua `@MinLength(1)` dù về nội dung thực chất là rỗng. Đã sửa `CreateCommentDto.body` bằng `@Matches(/\S/, {...})` thay cho `@MinLength(1)` — `\S` đòi hỏi ít nhất 1 ký tự không-phải-khoảng-trắng ở bất kỳ đâu trong chuỗi, chặn được cả `""` lẫn chuỗi toàn dấu cách/tab/xuống dòng mà không cần 2 decorator chồng nhau (lưu ý: không chặn được ký tự Unicode không-in-được nhưng cũng không khớp `\S`, vd zero-width space `U+200B` — lỗ hổng này đã tồn tại từ trước với `@MinLength(1)`, không phải regression mới). Rà lại thấy `CreateArticleDto`/`UpdateArticleDto` (`title`/`description`/`body`) có cùng lỗ hổng — đã sửa đồng loạt sang `@Matches(/\S/, {...})`, không đợi review riêng.

**Áp dụng:** field free-text nào chỉ có `@MinLength(1)` để chặn "không được rỗng" (title/description/body/comment...) phải dùng `@Matches(/\S/, {...})` thay vì `@MinLength(1)` để chặn luôn trường hợp toàn khoảng trắng — áp dụng cho **mọi** field free-text trong repo, không chỉ chỗ reviewer vừa chỉ ra. Bộ ba `@IsString`/`@Matches(/\S/, ...)`/`@MaxLength` được gom vào 1 decorator dùng chung `IsNonBlankString(maxLength, messageKeys)` (`src/common/decorators/is-non-blank-string.decorator.ts`) thay vì lặp lại ở từng field — field mới chỉ cần `@IsNonBlankString(MAX_X_LENGTH, { minLength: '...', maxLength: '...' })`.
