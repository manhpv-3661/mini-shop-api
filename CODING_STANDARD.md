...
| `C041`                                         | _"Variable 'NOTIFICATION_SECRET_KEY_PROVIDER' appears to contain hardcoded sensitive information"_ trong `notifications.constants.ts`                                                                                     | Đây là DI token (chuỗi định danh provider, giống `REDIS_CLIENT` đã có sẵn), không phải secret thật — giá trị chuỗi trùng tên biến chỉ để làm token duy nhất cho `@Inject()`. Heuristic thấy cụm `SECRET_KEY` trong tên biến nên tưởng là credential bị hardcode.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
